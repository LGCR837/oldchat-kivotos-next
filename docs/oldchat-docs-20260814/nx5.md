...
signingString = token + '\n' + path + '\n' + ts + '\n' + nonce
// 即: access_token原文 + 换行 + 路径(不含query) + 换行 + 秒级时间戳 + 换行 + nonce
```

**⚠️ 重要**: 签名使用的是 **token原文**（不带 "Bearer " 前缀），不是 HTTP method！
...
**X-Session 不参与签名计算**。签名只覆盖 `token + path + ts + nonce`。

但 X-Session 是**必须的请求头**（服务端用它查找 macKey 来验签）。`h0/e.java` 第190行：客户端在签名时从 `AbstractC0588y.b(context)` 获取 device_id 单独设置 `X-Device-Id` 头，也不参与签名。
...
nx3 §9.4 写 `signingString = token + "\n" + path + "\n" + ts + "\n" + nonce` 是**正确的**。但 nx3 说"X-Session 只当 header 不进签名"容易误解——实际上 X-Session 是独立的请求头，签名公式里确实不含它，但服务端**需要 X-Session 来查找 macKey**，缺少会返回 `bad_signature`。
