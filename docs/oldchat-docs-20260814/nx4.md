...
X-Ts: <unix 秒>
X-Nonce: <16字节随机 → Base64 NO_PADDING|NO_WRAP>
X-Sign: <HMAC-SHA256 → Base64 NO_PADDING|NO_WRAP>
...
**注意**: `X-Device-Id` **不参与签名计算**，仅用于设备识别。

### 9.5 两个传输层

| 层 | 类 | 签名 (X-Sign) | 加密 (X-Enc) |
|---|---|---|---|
| 旧传输层 | `h0.e` | ✅ 有 | ❌ 无 |
| 新传输层 | `h0.c` | ❌ 无 | ✅ 有 |
...
```java
// h0.e 第661行
return System.currentTimeMillis() >= f7208c
    && str != null
    && !str.startsWith("/auth/handshake")
    && AbstractC0584w.u(); // ECDH 会话已建立
```

**所有非 `/auth/handshake` 路径** 都可能被加密。
