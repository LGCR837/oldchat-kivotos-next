...
### 5.1 三层加密架构 (v1.4.x, jadx 确认)

```
┌─────────────────────────────────┐
│       应用层加密 (E2EE)          │  ECDH + AES
├─────────────────────────────────┤
│       传输层加密 (TLS)           │  Conscrypt TLS 1.0-1.3 ⭐
├─────────────────────────────────┤
│       传输层                     │  TCP
└─────────────────────────────────┘
```
...
- **传输层**: Conscrypt TLS (v1.4.x升级，156个Java文件，支持 TLS 1.0-1.3)

### 5.2 v1.4.x 安全层升级 (jadx 确认)

v1.4.x 引入 Conscrypt TLS 库后，传输层安全性显著提升:

| 方面 | v1.3.61 | v1.4.x |
|---|---|---|
| TLS实现 | 系统默认 | Conscrypt (156个Java文件) |
| TLS 1.3 | 依赖系统 | 全版本支持 |
| 加密加速 | 软件 | JNI硬件加速 (libconscrypt_jni.so) |
| 证书透明度 | 不支持 | 支持 |
| 一致性 | 设备差异 | 统一实现 |
...
| 攻击向量 | 防护 | v1.4.x 改进 |
|---|---|---|
| 窃听 (Eavesdropping) | TLS + E2EE | Conscrypt TLS更强 |
| 中间人 (MITM) | TLS | CT支持增强防护 |
| 服务端泄露 | E2EE 保护消息内容 | 不变 |
| 设备丢失 | Token/密钥存储安全 | 不变 |
...
| 密钥验证 | 未知 | 安全号码 | 指纹 |
| 传输层 | Conscrypt TLS | TLS | MTProto |
| 开源 | 否 | 是 | 是 |

OldChat 的 ECDH 实现细节由于代码混淆难以完全确认，但从 SpongyCastle 的使用和相关类的结构来看，至少实现了基本的 ECDH 密钥协商。v1.4.x 引入的 Conscrypt TLS 进一步增强了传输层安全性。
