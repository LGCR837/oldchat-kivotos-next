...
> **基于 `oldchat-1.4.5-540.apk` (2026-08-29) jadx 反编译分析**  
> **APK 大小**: 5.0 MB | **DEX 大小**: 6.3 MB | **Java 源文件**: 220+ 个  
> **包名**: `com.im.oldchat` | **版本**: 1.4.5 (versionCode 540)  
> **对照资料**: nx9 逆向文档、mcl0 官方文档 (api.md / client-guide.md / in_api.md / routes.md)  
> **基础版本**: nx9 (2026-08-24)  
> **更新时间**: 2026年8月29日  
> **更新说明**: nx10 在 nx9 基础上新增 §60-§68 共 9 个深度剖析章节（刮刮乐系统、CIP应用商店、前端昵称/称号HSV哈希着色体系、Safe控件与稳定性、v1.4.5新增API端点总表、数据库变更、后端接口改动分析、完整设置项清单、官方更新日志逐条分析），并对 §1/§5/§7 进行版本更新
...
60. [刮刮乐系统](#60-刮刮乐系统) **[NEW]**
61. [CIP 应用商店](#61-cip-应用商店) **[NEW]**
62. [前端效果: 聊天昵称/称号 HSV 哈希着色体系](#62-前端效果-聊天昵称称号-hsv-哈希着色体系) **[NEW]**
63. [Safe 控件与稳定性](#63-safe-控件与稳定性) **[NEW]**
64. [v1.4.5 新增 API 端点总表](#64-v145-新增-api-端点总表) **[NEW]**
65. [数据库变更](#65-数据库变更) **[NEW]**
66. [后端接口改动分析](#66-后端接口改动分析) **[NEW]**
67. [完整设置项清单](#67-完整设置项清单) **[NEW]**
68. [官方更新日志逐条分析](#68-官方更新日志逐条分析) **[NEW]**
...
| 属性 | 值 | 来源 |
|---|---|---|
| 包名 | `com.im.oldchat` | DEX strings / AndroidManifest |
| APK 文件名 | `oldchat-1.4.5-540.apk` | — |
| APK 大小 | 5.0 MB (5,123,669 bytes) | 文件系统 |
| DEX 文件 | `classes.dex` (单 DEX, 6.3 MB) | 文件系统 |
| Activities | 84 个 (含抽象基类) | jadx 枚举 |
| Services | 4 个 | jadx 枚举 |
| 自定义控件 | 16 个 | jadx 枚举 |
| Java 源文件 | 220+ 个 | jadx 输出 |
| **版本名** | **1.4.5** | AndroidManifest |
| **版本号** | **540** | AndroidManifest |
...
│   ├── DiscoverTileEditorActivity.java — 发现页编辑
│   ├── AppearancePreviewActivity.java — 外观预览
│   ├── WelcomeGuideActivity.java — 欢迎引导
│   ├── CipStoreActivity.java  — CIP 应用商店 **[NEW]**
│   ├── ScratchActivity.java   — 每日刮刮乐 **[NEW]**
│   ├── ReportActivity.java    — 举报提交 **[NEW]**
│   ├── widget/                — 自定义控件 (16个)
│   ├── SafeGridLayoutManager.java — 安全网格布局 **[NEW]**
│   ├── SafeLinearLayoutManager.java — 安全线性布局 **[NEW]**
│   ├── SafeRecyclerView.java  — 安全列表 **[NEW]**
│   │   ├── ButtonFlowLayout.java — 按钮流式布局
│   │   ├── DiscoverTileLayout.java — 发现页磁贴布局
│   │   ├── BubbleTimeTextView.java — 气泡时间文本
│   │   ├── MomentImageView.java — 动态图片视图
│   │   └── ... (9个已有控件)
│   └── ... (70+ Activities)
├── service/                   — 后台服务
...
| `CipStoreActivity` | CIP 应用商店 | **[NEW nx10]** |
| `ScratchActivity` | 每日刮刮乐 | **[NEW nx10]** |
| `ReportActivity` | 举报提交 | **[NEW nx10]** |
...
| `SafeGridLayoutManager` | 安全网格布局 (防越界崩溃) **[NEW nx10]** |
| `SafeLinearLayoutManager` | 安全线性布局 (防越界崩溃) **[NEW nx10]** |
| `SafeRecyclerView` | 安全列表 (防触摸事件异常崩溃) **[NEW nx10]** |
...
*本文档基于 `oldchat-1.4.5-540.apk` (2026-08-29) jadx 反编译 + DEX strings 全量分析，所有字段名均经源码验证。*
*§1-§59 内容基于 nx9 文档 (2026-08-24)，§60-§68 为 v1.4.5-540 新增内容。*
...
> **来源**: DEX strings 分析 (`ScratchActivity.java` 6个内部类) + in_api.md §16.2-16.3

### 60.1 功能概述

每日刮刮乐小游戏，用户每天可刮开 5 个槽位获得旧币奖励。

### 60.2 接口详情

**查询状态**: `GET /v2/me/scratch`
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `already_scratched` | boolean | 今日是否已刮 |
| `scratch_date` | String | 刮奖日期 (YYYY-MM-DD) |
| `slots` | int[] | 5 个槽位的奖励值 |
| `total_reward` | int | 本次总奖励 |
| `coin_balance` | int | 当前旧币余额 |

**开奖**: `POST /v2/me/scratch`

请求: `{}`

响应: 同 GET。每日仅限 1 次，重复调用返回已开奖结果。

**v1 兼容**: `GET/POST /me/scratch`
...
> **来源**: DEX strings 分析 (`CipStoreActivity.java` 12个内部类) + nx9-incremental.md

### 61.1 功能概述

CIP 小程序应用商店，支持浏览、搜索、安装 CIP 小程序。

### 61.2 接口详情

**获取商店列表**: `GET /v2/cip/store` (v2 加密路由)

**v1 兼容**: `GET /cip/store`

**上传 CIP 包**: `POST /cip-upload`

Multipart: `file` (CIP ZIP 包)

### 61.3 CipStoreActivity 结构

12 个内部类，说明这是一个功能完整的页面:
...
| 组件 | 功能 |
|---|---|
| `MiniAppsActivity` | 已安装小程序列表 |
| `LuaMiniAppActivity` | 小程序运行容器 |
| `CipDeveloperActivity` | CIP IDE |
| `CipVibeCodingActivity` | AI 编程助手 |
| `CipDevelopmentModeActivity` | 开发模式入口 |
| **`CipStoreActivity`** | **应用商店 (新增)** |
...
## 62. 前端效果: 聊天昵称/称号 HSV 哈希着色体系 **[NEW]**

> **来源**: DEX strings 深度分析 (HSVToColor, colorToHSV, ForegroundColorSpan, StyleSpan, SpannableStringBuilder, group_colored_name_enabled, sender_name_outside_bubble_enabled, user_title_cache, group_role_cache, 硬编码颜色值, hashSlot)
> 
> **注意**: 这里说的是「昵称」(display_name) 和「称号」(user_title)，不是「头衔」。
> 每个人的昵称和称号颜色**各不相同**，这是前端 HSV 哈希着色效果。

### 62.1 颜色体系架构: HSV 哈希着色

聊天界面中，每个人的昵称和称号颜色**各不相同**，这不是简单的角色固定色，而是基于 **HSV 色彩空间的哈希着色算法**。

**核心证据**: DEX strings 中发现了 `HSVToColor` 和 `colorToHSV`，说明颜色是通过 HSV 色彩空间计算的。
...
│                                                             │
│  3. 角色特殊覆盖 (可选)                                      │
│     ├→ 群主 (role=2): 使用固定红色 #FF5B6E                  │
│     ├→ 管理员 (role=1): 使用固定金色 #FFC857                │
│     └→ 普通成员: 使用 HSV 哈希色 (每人不同)                  │
│                                                             │
│  4. 称号 (user_title) 着色                                  │
│     └→ 与昵称使用相同的 HSV 哈希算法                         │
│        (每个用户的称号颜色也各不相同)                         │
...
从 DEX strings 中提取到的所有硬编码颜色值:

| 颜色 Hex | 用途 | 视觉效果 | 使用场景 |
|---|---|---|---|
| `#FF5B6E` | 群主昵称固定色 | 红色/珊瑚红 | 群聊发送者昵称 |
| `#FFC857` | 管理员昵称固定色 | 金色/琥珀色 | 群聊发送者昵称 |
| `#6FACC9` | UI 强调色 | 天蓝色 | 链接、按钮等 |
| `#4D8BFF` | 通用强调色 | 蓝色 | 链接、按钮等 |
| `#5B6E8A` | 次要文本 | 灰蓝色 | 辅助信息 |
| `#8A3A4A` | 暗红色调 | 深红 | 特殊标记 |
| `#8A5B6E` | 暗紫色调 | 紫灰 | 特殊标记 |
| `#7A5C2A` | 暗金色调 | 棕金 | 特殊标记 |
| `#8C837A` | 灰色调 | 灰色 | 次要元素 |
| `#B5ABA0` | 浅灰色调 | 浅灰 | 次要元素 |
| `#1A1F26` | 深色背景 | 深蓝黑 | 暗色主题 |
| `#1C1A17` | 暗色背景 | 深棕黑 | 暗色主题 |
| `#33507A` | 中蓝色 | 钢蓝 | UI 元素 |
| `#3A3733` | 深灰色 | 深灰 | UI 元素 |
| `#5CFFFFFF` | 半透明白 (36%) | 半透明白 | 叠加层 |
| `#B3FFFFFF` | 半透明白 (70%) | 半透明白 | 叠加层 |
...
| 关闭效果 | 所有昵称和称号使用系统默认颜色 |

#### `sender_name_outside_bubble_enabled` — 发送者名称位置
...
| 排序优先级 | 字段 | 方向 | 说明 |
|---|---|---|---|
| 1 | `role` | DESC | 群主(2) → 管理员(1) → 成员(0) |
| 2 | `display_name` | ASC (不区分大小写) | 显示名 |
| 3 | `username` | ASC (不区分大小写) | 用户名 |
| 4 | `ncuid` | ASC | NCUID 兜底 |
...
### 63.1 SafeGridLayoutManager

- 继承: `GridLayoutManager`
- 功能: 重写 `onLayoutChildren()` 等方法，捕获 `IndexOutOfBoundsException`
- 场景: 快速滚动、数据变更时的越界保护

### 63.2 SafeLinearLayoutManager

- 继承: `LinearLayoutManager`
- 功能: 同上，针对线性布局

### 63.3 SafeRecyclerView

- 继承: `RecyclerView`
- 功能: 可能重写 `onTouchEvent`、`onInterceptTouchEvent` 等方法
- 场景: 触摸事件处理中的异常保护
...
- 不影响正常功能
- 只在异常情况下降级 (catch → 不崩溃)
- 是 RecyclerView 最常见崩溃 (`IndexOutOfBoundsException`) 的标准解决方案

---

## 64. v1.4.5 新增 API 端点总表 **[NEW]**

### 64.1 全新端点

| 端点 | 方法 | 版本 | 说明 |
|---|---|---|---|
| `/v2/me/scratch` | GET | v2 | 查询今日刮刮乐状态 |
| `/v2/me/scratch` | POST | v2 | 每日刮刮乐开奖 |
| `/me/scratch` | GET/POST | v1 | 刮刮乐 (v1 兼容) |
| `/v2/cip/store` | GET | v2 | CIP 商店应用列表 |
| `/cip/store` | GET | v1 | CIP 商店 (v1 兼容) |
| `/cip-upload` | POST | v1 | CIP 包上传 |
| `/v1/chat/completions` | POST | v1 | AI 对话备用路径 |
| `/chat-context?group_id=` | GET | v1 | 群聊上下文查询 |
| `/me/resource-reports?limit=50` | GET | v1 | 资源举报查询 |

### 64.2 已有端点确认 (nx9 中推测，本次确认)

| 端点 | 说明 | 确认状态 |
|---|---|---|
| `/v2/channels/events/after` | 频道事件增量 | ❌ 客户端未调用 |
| `/v2/groups/events/after` | 群事件增量 | ❌ 客户端未调用 |
| `/v2/channel-api/apply` | 频道 Publisher Token | ❌ 客户端未调用 |
| `/v2/channel-api/status` | Publisher Token 状态 | ❌ 客户端未调用 |

> 这些端点在 api.md 中有定义，但 Android 客户端未调用。可能是 Web 端专用或服务端管理接口。

---

## 65. 数据库变更 **[NEW]**

### 65.1 members_live 新增索引
...
**作用**: 加速按 uid 查询群成员，特别是在聊天界面渲染时根据 `from_uid` 查找成员角色和称号。
...
**新增**: `/v2/me/scratch` GET/POST

**后端实现**:
- 需要 `scratch_date` 字段记录日期
- 5 个槽位独立随机
- 概率: 40% (0) / 30% (1) / 15% (5) / 10% (10) / 5% (20)
- 奖励累加到 `coin_balance`
- 每日 00:00 重置

### 66.2 CIP 应用商店

**新增**: `/v2/cip/store`, `/cip/store`, `/cip-upload`

**后端实现**:
- CIP 包存储和元数据管理
- 包校验 (manifest.json + main.lua)
- 下载计数
- 包大小限制

### 66.3 AI 备用路径

**新增**: `/v1/chat/completions`

**后端实现**: 路由到现有 `/ai/chat/completions` handler

### 66.4 群聊上下文

**新增**: `/chat-context?group_id=`

**后端实现**: 需实测确认具体响应结构

### 66.5 资源举报

**新增**: `/me/resource-reports?limit=50`

**后端实现**: 与 `/me/user-reports`、`/me/group-reports` 并列

### 66.6 成员索引优化

**变更**: `members_live` 表新增 `uid` 索引
...
| `hide_avatar_enabled` | boolean | **极速纯净模式 (不显示头像)** |
...
### 68.3 Android 2.3 Keep-Alive 连接断流修复
