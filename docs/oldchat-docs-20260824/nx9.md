...
> **基于 `oldchat-dev.apk` (2026-08-15 最新版) jadx 反编译分析**  
> **APK 大小**: 4.9 MB | **DEX 大小**: 6.2 MB | **Java 源文件**: 206 个  
> **包名**: `com.im.oldchat` | **版本**: dev 分支  
> **对照资料**: nx3 逆向文档、mcl0 官方文档 (api.md / client-guide.md / lua-cip.md / routes.md)  
> **基础版本**: nx8 (2026-08-15)  
> **更新时间**: 2026年8月24日  
> **更新说明**: nx9 在 nx8 基础上新增 §35-§50 共 16 个深度剖析章节（消息系统、群聊同步、图片加载、URL解析、Token刷新、WS管线、聊天页生命周期、首页列表、v1接口、注册流程、群聊接口、频道接口、朋友圈接口、文件系统、事件系统、安全防护），原 §35-§43 重编号为 §51-§59
...
35. [消息系统深度剖析](#35-消息系统深度剖析) **[NEW]**
36. [群聊可靠同步机制](#36-群聊可靠同步机制) **[NEW]**
37. [图片加载系统](#37-图片加载系统) **[NEW]**
38. [媒体URL解析与下载选路](#38-媒体url解析与下载选路) **[NEW]**
39. [Token刷新与认证安全](#39-token刷新与认证安全) **[NEW]**
40. [WebSocket消息处理管线](#40-websocket消息处理管线) **[NEW]**
41. [聊天页生命周期管理](#41-聊天页生命周期管理) **[NEW]**
42. [首页与聊天列表](#42-首页与聊天列表) **[NEW]**
43. [v1 独有接口补全](#43-v1-独有接口补全) **[NEW]**
44. [注册与验证码流程](#44-注册与验证码流程) **[NEW]**
45. [群聊完整接口补全](#45-群聊完整接口补全) **[NEW]**
46. [频道完整接口补全](#46-频道完整接口补全) **[NEW]**
47. [朋友圈完整接口补全](#47-朋友圈完整接口补全) **[NEW]**
48. [文件系统完整补全](#48-文件系统完整补全) **[NEW]**
49. [事件系统完整补全](#49-事件系统完整补全) **[NEW]**
50. [安全防护完整补全](#50-安全防护完整补全) **[NEW]**
51. [新闻模块](#51-新闻模块)
52. [发现页磁贴编辑](#52-发现页磁贴编辑)
53. [收藏夹](#53-收藏夹)
54. [二维码名片与扫码](#54-二维码名片与扫码)
55. [外观预览](#55-外观预览)
56. [分页约定](#56-分页约定)
57. [限流与错误信封](#57-限流与错误信封)
58. [v1/v2 端点版本总表](#58-v1v2-端点版本总表)
59. [仍无法确定 / 需实测后端才能确认](#59-仍无法确定--需实测后端才能确认)
...
**源码确认**: `LuaMiniAppActivity.java` (1831行) + `MiniAppsActivity.java` (417行) + `CipVibeCodingActivity.java` (776行) + `CipDeveloperActivity.java` (433行) + `CipVibeBackgroundService.java` + `welcome.lua`
...
**限制**:
- 压缩文件最大 2 MiB，解压后最大 8 MiB
- 最多 128 个文件
- `assets/` 仅允许: `.png`, `.jpg`, `.jpeg`, `.webp`, `.gif`, `.json`, `.txt`, `.md`
- 禁止绝对路径、`..`、路径穿越

### 20.2 manifest.json 完整字段
...
| 字段 | 必需 | 类型 | 说明 |
|---|---|---|---|
| `id` | ✅ | String | 1-64位，小写字母/数字/`_`/`-` |
| `name` | ✅ | String | 显示名，≤40字符 |
| `description` | ❌ | String | 描述，≤200字符 |
| `version` | ✅ | int | 正整数，变更时递增 |
| `icon_url` | ❌ | String | `assets/...` 或 HTTP(S) URL |
| `enabled` | ✅ | boolean | `true` 才下发到客户端 |
| `order` | ❌ | int | 数字越小越靠前 |
| `permissions` | ✅ | String[] | 权限数组 |
| `allowed_hosts` | ❌ | String[] | 外网白名单，最多32项，`*`=全部 |

### 20.3 权限系统

**源码确认**: `LuaMiniAppActivity.I0()` 方法

| 权限 | 说明 | 影响的 API |
|---|---|---|
| `network` | 同服务器 HTTP | `app.http_get` (相对路径) |
| `network_external` | 公网 HTTP/HTTPS | `app.http_get` / `app.http_request` (绝对URL) |
| `storage` | 本地 KV 存储 | `app.storage_*` 系列 |
| `camera` | 相机拍照 | `app.camera` |

### 20.4 Lua 运行时

**引擎**: LuaJ 3.0.1 (`org.luaj.vm2`)

**已加载库** (`LuaMiniAppActivity.U0()` + 全局注册):
- `BaseLib` — 基础库 (print, pcall, error, type, tostring, tonumber, etc.)
- `StringLib` — 字符串库
- `TableLib` — 表库
- `MathLib` — 数学库
- `Bit32Lib` — 位运算库
...
**已删除的危险库**:
- `io`, `os`, `debug`, `package`, `require`, `dofile`, `loadfile`, `luajava`

### 20.5 UI DSL 控件清单

**源码确认**: `LuaMiniAppActivity.U0()` 第1482行

注册的14个控件类型:
```java
String[] strArr = {"page", "text", "button", "image", "input", 
  "checkbox", "list", "column", "row", "scroll", 
  "card", "divider", "progress", "spacer"};
```
...
**渲染**: `LinearLayout` (垂直) 包裹 ScrollView

| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `title` | String | "" | 页面标题 |
| `children` | Table | — | 子控件数组 |
| `background` | String | — | 背景颜色 |

#### 20.5.2 ui.text

```lua
ui.text({ id="status", text="你好", size=15, color="#6FACC9", center=true, margin=6 })
```
...
| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `id` | String | — | 控件ID (用于 app.set_text 等) |
| `text` | String | "" | 文本内容 |
| `size` | float | 15.0 | 字号 (sp) |
| `color` | String | 系统默认 | 颜色 (hex) |
| `center` | boolean | false | 居中 |
| `gravity` | String | "start" | 对齐: start/center/end |
| `margin` | int | 4 | 外边距 (dp) |
| `padding` | int | 10 | 内边距 (dp) |
| `padding_vertical` | int | 8 | 垂直内边距 |
| `width` | String | "match" | 宽度: match/wrap |
| `height` | int | 0 | 高度 (dp, 0=自适应) |
| `weight` | float | 0 | 权重 (flex布局) |
| `background` | String | — | 背景颜色 |
| `corner` | int | 0 | 圆角 (dp) |
| `stroke` | int | 0 | 边框宽度 |
...
```lua
ui.button({ text="点击", on_click=function() app.toast("OK") end })
```
...
```lua
ui.image({ url="https://example.com/img.jpg", height=180, on_click=function() end })
```

**渲染**: `ImageView` (ScaleType.CENTER_CROP)

| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `url` | String | "" | 图片URL (http/https/content:///file://) |
| `height` | int | 200 | 高度 (dp) |
| `on_click` | Function | — | 点击回调 |
| `margin` | int | 4 | 外边距 |
...
```lua
ui.input({ id="keyword", hint="请输入", text="", single_line=false, input_type="text", max_length=2000, on_change=function(v) end, on_focus_lost=function(v) end, on_submit=function(v) end })
```
...
| `hint` | String | "" | 占位提示 |
| `text` | String | "" | 初始文本 |
| `single_line` | boolean | false | 单行模式 |
| `input_type` | String | "text" | 类型: text/password/number/email/phone |
| `max_length` | int | 0 | 最大长度 (0=不限, ≤100000) |
| `on_change` | Function | — | 文本变化回调 (参数: 新文本) |
| `on_focus_lost` | Function | — | 失焦回调 (参数: 当前文本) |
| `on_submit` | Function | — | 提交回调 (参数: 当前文本, 仅single_line=true) |

**input_type 映射** (源码确认):
- `"text"` → InputType.TYPE_CLASS_TEXT
- `"password"` → 129 (TYPE_CLASS_TEXT | TYPE_TEXT_VARIATION_PASSWORD)
- `"number"` → 12290 (TYPE_CLASS_NUMBER)
- `"email"` → 33 (TYPE_CLASS_TEXT | TYPE_TEXT_VARIATION_EMAIL_ADDRESS)
- `"phone"` → 3 (TYPE_CLASS_PHONE)

#### 20.5.6 ui.checkbox

```lua
ui.checkbox({ id="remember", text="记住", checked=false, on_change=function(checked) end })
```
...
| `on_change` | Function | — | 状态变化回调 (参数: boolean) |

#### 20.5.7 ui.list / ui.column / ui.row / ui.scroll / ui.card

```lua
ui.list({ children = { ... } })
```

**渲染**: `LinearLayout` (list/column=垂直, row=水平)
...
- `ui.scroll`: ScrollView 包裹的垂直布局
- `ui.card`: 带圆角/边框/背景的容器

#### 20.5.8 ui.divider

```lua
ui.divider({ height=1, color="#EEEEEE" })
```
...
```lua
ui.progress({ value=50, max=100, color="#2196F3" })
```
...
**源码确认**: `LuaMiniAppActivity` 第1446-1476行

#### 20.6.1 资源

| API | 参数 | 返回 | 说明 |
|---|---|---|---|
| `app.asset(path)` | String | String | 返回包内资源URL (服务端包→`/lua-assets/<id>/...`, 本地包→沙箱URI) |

#### 20.6.2 提示

| API | 参数 | 返回 | 说明 |
|---|---|---|---|
| `app.toast(msg)` | String | — | 短暂提示 |
| `app.back()` | — | — | 关闭小程序 |

#### 20.6.3 本地存储 (需要 `storage` 权限)

| API | 参数 | 返回 | 说明 |
|---|---|---|---|
| `app.storage_get(key)` | String | String/nil | 读取 |
| `app.storage_set(key, value)` | String, String | true | 写入 |
| `app.storage_remove(key)` | String | true | 删除单个键 |
| `app.storage_clear()` | — | true | 清空当前应用存储 |
| `app.storage_keys()` | — | Table | 所有键名列表 |
| `app.storage_count()` | — | int | 键值对数量 |
| `app.storage_get_json(key)` | String | Table/nil | 读取并解析为JSON table |
| `app.storage_set_json(key, value)` | String, Table | true | 编码为JSON并写入 |

**存储隔离**: 每个应用使用 `SharedPreferences("lua_<app-id>")`，互不访问。

#### 20.6.4 控件操作

| API | 参数 | 返回 | 说明 |
|---|---|---|---|
| `app.get_text(id)` | String | String/nil | 获取控件文本 |
| `app.set_text(id, text)` | String, String | true | 设置文本 (主线程) |
| `app.append_text(id, text)` | String, String | true | 追加文本 |
| `app.set_hint(id, hint)` | String, String | true | 设置输入框提示 |
| `app.focus(id)` | String | — | 聚焦控件 |
| `app.get_checked(id)` | String | boolean | 获取复选框状态 |
| `app.set_checked(id, checked)` | String, boolean | true | 设置复选框 |
| `app.set_image(id, url)` | String, String | true | 设置图片URL |
| `app.set_visible(id, visible)` | String, boolean | true | 显示/隐藏 |
| `app.get_visible(id)` | String | boolean | 是否可见 |
| `app.set_enabled(id, enabled)` | String, boolean | true | 启用/禁用 |

#### 20.6.5 JSON

| API | 参数 | 返回 | 说明 |
|---|---|---|---|
| `app.json_decode(text)` | String | Table/nil | JSON → Lua table (最大嵌套12层, 最大4096项) |
| `app.json_encode(value)` | Table | String | Lua table → JSON |

#### 20.6.6 延时

| API | 参数 | 返回 | 说明 |
|---|---|---|---|
| `app.delay(ms, fn)` | int, Function | — | 延时执行, 最大60秒 |

#### 20.6.7 HTTP (需要 `network` 或 `network_external` 权限)

**app.http_get** — GET 请求

```lua
app.http_get("/me", function(body, err)
  if body then app.set_text("status", body)
  else app.toast(err or "请求失败") end
end)
```

**实现** (`C0183e.call()` 第527行):
- 路径以 `/` 开头 → 同服务器API，自动携带 `Authorization: Bearer <token>`
- 路径以 `http://` 或 `https://` 开头 → 公网请求 (需要 `network_external` 权限)
- 禁止 `://` 和 `..` 在相对路径中
- 响应通过 `h0.d.X()` (GET) 发送

**app.http_request** — 通用 HTTP 请求
...
**实现** (`C0184f.invoke()` 第576行):

| 请求属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `url` | String | — | 必须是 http/https 绝对URL |
| `method` | String | "GET" | HTTP 方法 |
| `headers` | Table | {} | 请求头 |
| `body` | String | "" | 请求体 |
| `json` | Table | — | JSON请求体 (自动设置 Content-Type) |
| `connect_timeout` | int | 10000 | 连接超时 (ms) |
| `read_timeout` | int | 25000 | 读取超时 (ms) |

#### 20.6.8 相机 (需要 `camera` 权限)

```lua
app.camera(function(uri, err)
  if uri then app.set_image("preview", uri)
  else app.toast(err or "拍照失败") end
end)
```

**实现**: 调用系统相机 Intent，结果通过 `MediaStore` 获取 URI。

#### 20.6.9 URL 编码

| API | 参数 | 返回 | 说明 |
|---|---|---|---|
| `app.url_encode(str)` | String | String | URL编码 |

### 20.7 控件 ID 系统

**源码确认**: `LuaMiniAppActivity.f3581E` (HashMap<String, View>)

所有带 `id` 属性的控件注册到全局 HashMap 中。`app.set_text(id, ...)` 等 API 通过此 HashMap 查找控件并操作。

控件操作通过 `runOnUiThread()` 确保在主线程执行。

### 20.8 回调机制

**源码确认**: `LuaMiniAppActivity.g1()` 方法

所有 Lua 回调 (on_click, on_change, on_focus_lost, on_submit, http_get callback 等) 都通过 `g1()` 方法执行:
- 在 Lua 线程中执行 (非主线程)
- 捕获异常防止崩溃
- UI 操作通过 `runOnUiThread()` 切换到主线程

### 20.9 客户端 API 端点

| 端点 | 方法 | 说明 |
|---|---|---|
| `GET /discover/lua/manifest` | GET | 远程小程序清单 |
| `GET /discover/lua/apps/{id}` | GET | 获取应用脚本 |
| `GET /lua-assets/{id}/{path}` | GET | 获取包内资源 |

### 20.10 MiniAppsActivity — 小程序列表

**功能**:
- 显示已安装的小程序列表
- 支持固定(pinned)小程序到发现页
- 本地导入 CIP 包 (ACTION_OPEN_DOCUMENT / ACTION_GET_CONTENT)
- 跳转到 Lua 开发文档 (`/lua-cip.md`)

**固定小程序** 存储在 `SharedPreferences("discover_tile_layout")` 的 `pinned_cip_app_ids` 键中。

### 20.11 CipDeveloperActivity — CIP IDE

**功能**:
- 文件树 + 代码编辑器 + 预览
- 支持 `manifest.json` 和 `main.lua` 编辑
- 新建资源文件 (assets/ 目录)
- 测试运行 (跳转 LuaMiniAppActivity)
- 导出 ZIP 包
- `configChanges=keyboardHidden|orientation|screenSize` 避免旋转重建

**关键限制**:
- `manifest.json` 和 `main.lua` 不可删除/重命名
- 文件名校验: `j0.i.f(trim)` 必须合法
- 导出通过 `FileProvider` 生成 content:// URI

### 20.12 CipVibeCodingActivity — AI 编程助手

**功能**: 自然语言描述需求 → AI 生成 CIP 小程序代码

**AI 配置** (`SharedPreferences("cip_vibe_config")`):

| 键 | 类型 | 说明 |
|---|---|---|
| `use_oldchat` | boolean | true=使用 Oldchat 免费额度 |
| `base_url` | String | 自定义 API Base URL |
| `api_key` | String | API Key (Base64 编码存储) |
| `model_id` | String | 模型ID |

**Oldchat 免费额度模式**:
- Base URL: `{服务器}/ai/chat/completions`
- Model: `deepseek-v4-flash`
- 使用当前登录 token 作为 API Key

**自定义模式**: 用户可配置 OpenAI 兼容的 API 端点。

**工具白名单**: `read` / `write` / `edit` / `grep` / `run` / `test` / `run_test` / `tasklist` / `ask`

### 20.13 CipVibeBackgroundService — 后台服务

**通知渠道**: `cip_vibe_worker`
**通知ID**: 7361
**通知文案**: "VibeCoding 正在后台工作" / "工具可持续调用，点击可返回查看"

**停止 Action**: `com.im.oldchat.action.STOP_VIBE`

### 20.14 welcome.lua — 内置示例

APK 内嵌 `assets/lua_apps/welcome.lua` 作为默认欢迎小程序。

### 20.15 安全限制

**源码确认**: `LuaMiniAppActivity` 第529-598行

1. **路径校验**: `app.http_get` 的相对路径禁止 `://` 和 `..`
2. **公网隔离**: 相对路径只能访问同服务器 API，公网需 `network_external` 权限
3. **Token 保护**: 公网 HTTP 请求不携带登录 token
4. **JSON 限制**: 最大嵌套12层，单个对象/数组最大4096项
5. **存储隔离**: 每个应用独立 SharedPreferences
6. **UI 线程安全**: 所有控件操作通过 `runOnUiThread()`
7. **异常捕获**: 控件构建和回调执行都有 try-catch 保护
...
> **来源**: client-guide.md §3, 源码分析 (`models/Message.java`, `util/MessagePayload.java`, `DirectMessageSender.java`, `DirectPendingMessageHelper.java`, `DirectMessageMerger.java`, `DirectChatLoadDelegate.java`)

### 35.1 MessagePayload v2 完整字段

**源码确认**: `util/MessagePayload.java`

当 `msg_type = "text"` 且 body 以 `{` 开头时，解析为 v2 JSON：

```json
{
  "v": 2,
  "text": "消息文本",
  "media_kind": "image|voice|video|music|null",
  "voice_text": "语音转文字结果",
  "quote": { ... },
  "mentions": [ ... ],
  "forward_v2": { ... }
...
| `v` | int | 协议版本，固定为 2 |
| `text` | String | 消息文本内容 |
| `media_kind` | String | 媒体子类型标识 (image/voice/video/music) |
| `voice_text` | String | 语音转文字结果 |
| `quote` | Quote | 引用消息结构 |
| `mentions` | Mention[] | @提及列表 |
| `forward_v2` | ForwardBundle | 转发聊天记录结构 |

**兼容性**: `fromBody(body)` 解析时，旧版 `v=0/1` 自动降级为纯文本 (`extractFallbackText`)。

### 35.2 ForwardBundle 结构
  ...
  "items": [
    {
      ...
      "from_uid": "USR-XXX",
      "from_name": "张三",
      "from_avatar": "/v1/uploads/avatars/xxx.jpg",
      "type": "text",
      "media_kind": "image",
      "text": "消息内容"
    }
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `title` | String | 转发标题 (默认 "聊天记录") |
| `items[].source_message_id` | String | 原始消息ID |
| `items[].from_uid` | String | 原发送者UID |
| `items[].from_name` | String | 原发送者名称 |
| `items[].from_avatar` | String | 原发送者头像 |
| `items[].type` | String | 消息类型 |
| `items[].media_kind` | String | 媒体子类型 |
| `items[].text` | String | 消息文本 |
...
```json
{
  ...
  "text": "被引用消息内容（截取前200字符）",
  "media_kind": "image",
  "thumb_url": "/v1/uploads/media/thumb.jpg"
}
```
...
|---|---|---|
| `id` | String | 被引用消息ID |
| `from_uid` | String | 原发送者UID |
| `from_name` | String | 原发送者名称 |
| `type` | String | 原消息类型 |
| `text` | String | 原消息文本 (截取前200字符) |
| `media_kind` | String | 媒体子类型 |
| `thumb_url` | String | 缩略图URL |
...
> **注意**: 官方文档 (client-guide.md) 使用 `uid + name` 两字段；nx8 源码分析发现 `ncuid + uid + name` 三字段。两者都存在于实际代码中，`ncuid` 为兼容字段。

### 35.5 发送流程

**完整链路** (源码确认: `DirectMessageSender.java` + `DirectPendingMessageHelper.java`):

```
UI 输入 → sendText(content, quoteDraft)
  ↓
MessagePayloadBuilder.buildBody(text, quote, mentions, mediaKind)
  ↓ body = {"v":2,"text":"你好"}
DirectPendingMessageHelper.enqueueText(listHelper, myUid, body, burnAfterSeconds)
  ↓ 创建 Message{id=local_*, localPending=true, localRequestId=local_*}
  ↓ 加入 listHelper.messageList + messageIds
  ↓ DirectMessageMerger.sortMessages()
  ↓ trimOldestInMemory(MAX_ACTIVE_WINDOW_MESSAGES)
  ↓ 适配器标记 animating
  ↓ 更新 RecentChatCache
  ↓ notifyReloadWithStatus() (只刷新变化行)
  ↓ scrollToBottom()
POST /direct/send {to_uid, body, msg_type:"text", burn_after_seconds?}
  ↓ 成功
...
DirectPendingMessageHelper.completeText(listHelper, localId, sent)
  ...
  ↓ copyServerFields(pending, sent) (替换 id/created_at/sort_seq/status)
  ↓ localPending=false, localFailed=false, localRequestId=null
  ↓ 排序 + 刷新状态行
```

**失败处理**: `failText(listHelper, localId)` → `localPending=false`, `localFailed=true`
...
选图/拍照/录音
  ↓
enqueueImageSpinner / enqueueImageWithProgress (创建 pending 气泡, localPreviewUri=content://)
  ↓
压缩图片 (3MB 限制) + 生成缩略图
  ↓
POST /media (multipart/form-data, file + thumb?)
  ↓ 返回 {url, thumb_url?} — 两个 URL 都是 /v1/uploads/media/xxx 相对路径
更新 pending 的 media_url / thumb_url
  ↓
sendPendingMedia(type, url, thumbUrl, durationMs, body, burnSeconds, pendingLocalId)
  ↓
POST /direct/send {to_uid, body, msg_type, media_url, thumb_url, duration_ms, burn_after_seconds?}
  ↓
服务端确认 → completeText 替换 pending
```

### 35.7 文件发送流程

```
ChatFileUploadHelper.pickFile() → ACTION_GET_CONTENT
  ↓
queryDisplayName(uri) + querySize(uri) + resolveMime(uri)
  ↓ fileSize > 1GB → 拒绝
ChatFileUploadHttpClient.upload(activity, uploadUrl, uri, ...)
  ↓ setChunkedStreamingMode(0)
  ↓ 边读边累计 written, 超过 1GB 立即返回 413
  ↓ 返回 {url} 或 {download_path}
normalizeDownloadUrl(url, apiBase)
  ↓
sendFileResource(url, fileName, fileSize, quoteDraft)
  ↓ msg_type="resource", body=FileUploadUiTextUtil.buildBody(fileName, fileSize, url)
```

### 35.8 localPending 消息不持久化

**源码确认**: `MessageHistoryCache.java`

`MessageHistoryCache` 保存时过滤所有 `localPending=true` 的消息：
...
读取时也会调用 `removeStalePending` 清理遗留 pending。进程被杀后不会留下永久"发送中"气泡。

### 35.9 消息合并排序

**源码确认**: `DirectMessageMerger.java`

`sortMessages()` 排序规则:
1. `created_at` ASC (主排序)
2. `sort_seq` ASC (同秒排序，SQLite rowid)
3. ID 字符串比较 (兜底)

### 35.10 mergeRefresh 详细流程

**源码确认**: `DirectMessageMerger.mergeRefresh()`

```
mergeRefresh(list, ids, incoming, maxWindow, append):
  1. 按 ID 去重: incoming 中已有 ID 的跳过
  2. incoming 中 ID 在本地 local_* pending 的 → 替换 pending
     └→ findMatchingPendingIndex 宽松匹配 (仅文本, 唯一匹配)
  3. append 模式前插; refresh 模式合并
  4. trimOldestInMemory: 超过 MAX_ACTIVE_WINDOW_MESSAGES 时裁剪最早
     └→ 同时清理 messageIds
```

### 35.11 历史加载策略

**源码确认**: `DirectChatLoadDelegate.java`

**完整流程**:
1. **缓存与网络并行**: 首次进入时同时发起缓存读取和网络请求；网络先返回时缓存回调因列表非空自动丢弃
2. **v2 游标分页优先**: `/direct/messages/v2?with_uid=&limit=&offset=&before_created_at=&before_id=&anchor_message_id=`
3. **404 回退**: v2 返回 404 (旧服务端无此路由) → `legacyHistoryRoute=true` → 回退 `/direct/messages?with_uid=`
...
> **来源**: client-guide.md §4.2, 源码分析 (`GroupReliableSync.java`, `GroupSyncWatermarkStore.java`)

### 36.1 不变量

群聊同步的核心设计原则:

1. **WS 低延迟推送**: WebSocket 负责实时消息推送
2. **HTTP group_seq 补偿**: HTTP `/groups/messages/after` 按 `group_seq` 补偿，保证后台断线后最终不漏消息
3. **水位只在 HTTP 完整页面合并后才推进**: 只有在完整 HTTP 页面合并并关联到本地消息锚点后才推进水位
4. **WS 不推进水位**: 若 WS 中间漏包，推进后 HTTP 将永久跳过缺失消息
...
**正常补偿流程** (源码确认: `GroupReliableSync.pullAfter()`):
...
### 36.4 pullLegacyUntilOverlap 详细流程

**旧服务端兜底** (源码确认: `GroupReliableSync.pullLegacyUntilOverlap()`):
...
finish(helper, token):
  ...
  ├→ 统一 notifyDataSetChangedNow()
  ├→ updateRecentFromMessages
  ├→ saveGroupMessages
  └→ 若 reliableSyncPending (同步期间又收到新消息) → 重新 start()
```

### 36.6 GroupSyncWatermarkStore

**源码确认**: `GroupSyncWatermarkStore.java`
...
**水位失效条件**: `anchorMessageId` 不在当前活动消息列表中 (如清缓存后重启)，水位自动失效，触发 `pullLegacyUntilOverlap` 全量恢复。

---

## 37. 图片加载系统

> **来源**: client-guide.md §5.3, 源码分析 (`ImageLoader.java`, `ImageLoaderBitmapSupport.java`, `ImageLoaderDecodeSupport.java`)
...
┌─────────────────────────────────────────────────┐
│ 第1级: 内存 LruCache                              │
│   按设备内存分级: 24MB / 16MB / 8MB / 1MB          │
├─────────────────────────────────────────────────┤
│ 第2级: 磁盘缓存                                   │
│   TTL: 7天 | 上限: 48-128MB | 清理间隔: 2h         │
├─────────────────────────────────────────────────┤
│ 第3级: ImageDiskCacheHint (内存标记)              │
│   内存中标记磁盘是否已有该 key, 避免主线程 IO 检查  │
├─────────────────────────────────────────────────┤
│ 第4级: BitmapInFlight (并发合并)                  │
│   同 URL 并发请求合并, 避免重复下载                 │
└─────────────────────────────────────────────────┘
```
...
| 约束 | 说明 | 原因 |
|---|---|---|
| 不使用 `inBitmap` | `ImageLoaderDecodeSupport` 完全移除 `inBitmap`/`inMutable` 字段访问 | API 9 `NoSuchFieldError` + 图片串图 |
...
| 4 秒快速回退 | `shouldUseFastFailoverTimeout` 对 `files.*` 和 `60.*` 域名使用 4 秒超时 | 避免首线路卡住时图片空白十几秒 |
| `oc.*` 主站 | 使用正常 12/20 秒超时 | 主站更可靠, 给更多时间 |

### 37.4 头像加载优化

```
enqueueAvatarTarget(view, url)
  ↓
deliverPendingAvatarTargets()
  ↓ 合并同 URL 的多个头像请求, 一次下载分发给所有等待的 ImageView
```
...
> **来源**: client-guide.md §6.1, 源码分析 (`MediaUrlResolver.java`, `DownloadUtilStorageSupport.java`)

### 38.1 MediaUrlResolver 候选顺序

**源码确认**: `MediaUrlResolver.resolveCandidates()`
...
**关键**: `resolve(url)` 返回第一个候选; `resolveCandidates(url)` 返回全部; `resolveNextCandidate(current)` 返回下一个。
...
**源码确认**: `DownloadUtilStorageSupport.shouldAttachAuth()`

```
只对 OldChat 可信源站附加 Authorization: Bearer:
  - 当前 BASE_URL
  - oc.mcl0.dpdns.org
  - 60.205.94.101:8080
  - 154.8.227.219:9090
...
不携带 token 的:
  - files OSS (ocf.oss-cn-shanghai.aliyuncs.com)
  - 第三方 URL
```
...
{
  "sources": [
    "https://cn-sy1.rains3.com/oldchat",
...
> **来源**: client-guide.md §2.3, 源码分析 (`HttpAuthHelper.java`, `HttpUtilSupport0.java`)

### 39.1 Token刷新单飞 (Single-Flight)

**源码确认**: `HttpAuthHelper.java`
...
authGeneration 全局递增
  ↓
applyAuthResponse() 校验:
  - generation 是否匹配 (防止退出后旧刷新覆盖)
  - 预期 token 是否一致
```

### 39.2 invalidateAuthOperations()

```java
// 登录/退出/切换账号时调用
public static void invalidateAuthOperations() {
    authGeneration++;  // 使所有在途刷新结果失效
}
```

**效果**: 所有在途的 token 刷新请求完成后, `applyAuthResponse()` 发现 generation 不匹配, 丢弃结果。
...
POST /auth/refresh {refresh_token}
  ↓ 失败 (refresh_token 无效)
POST /auth/login {saved_username, saved_password}
  ↓ 使用 SP 中明文保存的密码重新登录
  ↓ 成功 → 更新 token
  ↓ 失败 → 累计失败次数
...
> **来源**: client-guide.md §2.4, 源码分析 (`SimpleWebSocketClient.java`, `WSManager.java`, `WSIncomingHandler.java`, `WSDispatchHelper.java`)

### 40.1 SimpleWebSocketClient 自实现 RFC 6455

**源码确认**: `SimpleWebSocketClient.java`

```
HTTP Upgrade 握手
  ↓ (带 Sec-WebSocket-Key, Sec-WebSocket-Version: 13)
帧读取线程
  ↓ 读取掩码帧 → 解掩码 → 处理文本帧 / ping/pong / close
  ↓ 读取线程首先解密, 再送入解析队列
  ↓ (避免断连清除密钥后无法解密)
写入: 客户端帧必须掩码
```

**限制**: 不处理分片帧和二进制帧 (服务端若扩展协议需同步升级)。
...
WSIncomingHandler:
  ├→ 兼容性二次解密
  ├→ JSON 解析
  │    → WSModels.DirectMessage / GroupMessage / Recall / Typing / Presence
  ├→ 修复字段 (ncuid 解析, 时间戳补全)
...
| 特性 | 说明 |
|---|---|
| 退避策略 | 1s → 60s 指数退避 + ±20% 抖动 |
| 401/403 处理 | 后台刷新 token → 重连 |
| 连接 generation | 每条连接有 generation, 旧连接回调被忽略 |
| 重连补偿 | 重连后执行未读补偿, 设置标志供聊天页 `onResume()` 强制刷新 |
...
|---|---|
| 回调线程 | 已在主线程 |
| 注册 | `onResume` → `addListener` |
| 注销 | `onPause` → `removeListener` |
| 销毁 | `onDestroy` → 清除 Handler 和控制器 |

---

## 41. 聊天页生命周期管理

> **来源**: client-guide.md §3.5, 源码分析 (`ChatActivity.java`, `DirectChatSupportDelegate.java`)

### 41.1 ChatActivity Intent 参数

| 参数 | 类型 | 说明 |
|---|---|---|
| `friend_id` | String | 好友数据库ID |
| `friend_uid` | String | 好友UID |
| `friend_name` | String | 好友名称 |
| `friend_avatar` | String | 好友头像URL |

### 41.2 onCreate

```
1. DirectChatListHelper 初始化
   ├→ 持有 messageList / adapter / RecyclerView
   └→ 设置 RecyclerView (ViewHolderPool, 稳定ID)
2. WSManager.addListener(wsListener)
3. 加载缓存 → 加载网络
4. scrollToBottom 修正注册 (0/80/320ms + post-layout scrollBy)
   └→ bottomScrollGeneration 可取消
```
...
```
1. 标记已读 (POST /direct/read {thread_id})
2. 暂停图片加载 (setInlineLoadsPaused(true))
```
...
2. 清除 Handler (防止泄漏)
3. 释放控制器
4. 取消底部修正 (cancelBottomScroll)
```

### 41.6 底部定位修正

**源码确认**: `DirectChatSupportDelegate.scrollToBottom()`

```
scrollToBottom():
  ├→ postScrollBy(gen, 0ms)    — 立即
  ├→ postScrollBy(gen, 80ms)   — 布局完成后
  └→ postScrollBy(gen, 320ms)  — 兜底

postScrollBy(gen, delayMs):
  └→ recyclerView.postDelayed(() → {
       if (gen != bottomScrollGeneration) return;  // 已被取消
...
**bottomScrollGeneration 自增时机** (取消待执行修正):
- 用户手动拖动
- 跳转未读消息
- 引用跳转
- 历史锚定
...
> **来源**: client-guide.md §7, 源码分析 (`ChatsFragment.java`, `RecentChatBuilder.java`, `CombinedChatAdapter.java`)

### 42.1 RecentChatBuilder.buildCombinedList()

**合并数据源**:
```
buildCombinedList(app, unreadNotifications)
...
### 42.2 CombinedChatAdapter DiffUtil

`CombinedChatAdapter` 使用 `DiffUtil` 精确刷新, 避免 `notifyDataSetChanged()` 全量刷新。

### 42.3 聊天列表排序

**排序规则**: 最新消息时间倒序 (`created_at` DESC)
...
WS 消息 → scheduleReload(220ms) 防抖
  ↓
loadRecentsAsync: 单线程 executor
  ↓ 合并最新数据
  ↓ RecentSnapshotStore.save
  ↓ 主线程 ui.renderSections
...
> **来源**: in/api.md §7.6, §8.18, §10.9

### 43.1 私聊管理接口

| 端点 | 方法 | 说明 |
|---|---|---|
| `DELETE /v1/direct/messages/{messageID}` | DELETE | 撤回私聊消息 (2分钟内) |
| `GET /v1/direct/messages/search` | GET | 搜索私聊消息 |
| `POST /v1/direct/messages/{messageID}/transcribe` | POST | 语音转文字 |
| `GET /v1/chats/{chatId}/typing` | GET | 查询对方正在输入状态 |

### 43.2 群聊管理接口

| 端点 | 方法 | 说明 |
|---|---|---|
| `DELETE /v1/groups/messages/{messageID}` | DELETE | 删除群消息 |
| `GET /v1/groups/messages/search` | GET | 搜索群消息 |
| `POST /v1/groups/messages/{messageID}/transcribe` | POST | 群语音转文字 |
| `GET /v1/groups/{groupId}/typing` | GET | 查询群内正在输入状态 |

### 43.3 资料杂项接口

| 端点 | 方法 | 说明 |
|---|---|---|
| `POST /v1/voice/asr` | POST | 语音识别 (multipart file → `{text}`) |
| `POST /v1/media` | POST | 媒体上传 (≤50MB) |
| `POST /v1/files/upload` | POST | 文件上传 (≤1GB) |
| `GET /v1/notifications?limit=&before=` | GET | 通知列表 (`before`=Unix毫秒) |
| `POST /v1/report` | POST | 通用举报 |
...
> **来源**: in/api.md §3.1-3.3

### 44.1 发送邮箱验证码

**端点**: `POST /v1/auth/email/send`
...
| `geetest_*` | ❌ | 极验 4.0 验证字段 |
| `turnstile_token` | ❌ | Cloudflare Turnstile 验证 |

**限制**: 每邮箱 120s 冷却, 验证码 10 分钟有效一次性。

### 44.2 用户注册

**端点**: `POST /v1/auth/web/register`
...
| 字段 | 规则 |
|---|---|
| `username` | 3-20位, 小写字母/数字/下划线 |
| `password` | ≥8 位 |
| `email` | 邮箱白名单: qq.com / 126.com / 163.com |

**注册成功后**:
- 自动加入默认官方群 `defaultRegistrationGroupID`
- 返回 `access_token` / `refresh_token` / `user_id` / `uid`

### 44.3 管理员直建用户

**端点**: `POST /v1/auth/direct-create`

管理员专用, 直接创建测试用户, 无需邮箱验证。
  ...
  "groups": [
    {
      ...
      "announcement_updated_at": 1785123456,
      "announcement_read_at": 1785123000,
      "member_count": 50,
      "member_version": 12,
      "message_seq": 1042,
      "role": 2
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `join_approval` | boolean | 是否需要审批入群 |
| `global_mute` | boolean | 全员禁言 |
| `announcement` | String | 群公告内容 |
| `announcement_mode` | String | 公告模式 |
| `announcement_updated_at` | long | 公告更新时间 |
| `announcement_read_at` | long | 公告已读时间 |
| `member_count` | int | 成员数 |
| `member_version` | int | 成员列表版本号 (用于增量同步) |
| `message_seq` | long | 最新消息序号 |
| `role` | int | 0=成员, 1=管理员, 2=群主 |

### 45.2 群成员列表

**端点**: `GET /v2/groups/members`
...
**单个频道**: `GET /v2/channels/state?channel_id=CHN-XXX`

**批量查询**: `GET /v2/channels/states?channel_ids=CHN-A,CHN-B,CHN-C`
...
|---|---|---|
| `body` | String | 文字内容 |
| `image_url` | String | 单图 URL (兼容旧版) |
| `image_urls` | String[] | 多图 URL 数组 (新版) |

### 47.2 v1 独有接口

| 端点 | 说明 |
|---|---|
| `GET /v1/moments` | 旧版时间线 (非分页) |
| `GET /v1/moments/v2` | 分页新结构 (游标分页) |
...
POST /v2/files/check {sha256, size_bytes}
  ├→ 命中: 秒传, 直接返回 file_id + url
  └→ 未命中: POST /v2/files/upload (multipart)
       └→ 服务端始终重新计算 SHA-256 (不信任客户端)
```
...
| 字段 | 说明 |
|---|---|
| `file` | 图片 ≤10MB (不压缩) / 视频 ≤200MB / 语音 |
| `thumb` | 缩略图 (可选) |
| `X-File-Size` | 文件大小 (Header) |
| `X-File-SHA256` | 文件 SHA-256 (Header) |
...
下载 URL 可带 `?ref=` 参数, 供客户端拼对象直链 (雨云等对象存储)。
...
| 事件类型 | 说明 |
|---|---|
| `DIRECT_MESSAGE_NEW` | 新私聊消息 |
| `DIRECT_READ` | 私聊已读 |
| `DIRECT_MESSAGE_RECALL` | 私聊撤回 |
| `GROUP_MESSAGE_NEW` | 新群消息 |
...
| `FRIEND_DELETE` | 好友删除 |
| `RED_PACKET_NEW` | 新红包 |
| `RED_PACKET_CLAIMED` | 红包被领取 |
| `MOMENT_NEW` | 新动态 |
| `MOMENT_LIKE` | 动态点赞 |
| `MOMENT_COMMENT` | 动态评论 |
| `CHANNEL_POST_NEW` | 频道新帖 |
| `CHANNEL_SUBSCRIBE` | 频道订阅 |
| `NOTIFICATION_NEW` | 新通知 |

### 49.2 群事件补差

**端点**: `GET /v2/groups/events/after?group_id=GRP-XXX&seq=N`
...
**端点**: `GET /v2/groups/messages/after`
...
请求进入
  ↓
secureMiddleware (第1层: 解密)
  ├→ X-Enc:1 时解密 body (AES-256-CBC + HMAC-SHA256)
  ├→ 解密 X-Auth 注入 Authorization
  └→ 大文件 multipart 豁免
  ↓
authMiddleware (第2层: 认证)
  ├→ JWT 签名校验
  ├→ iss (issuer) 校验
  ├→ exp (expiration) 校验
  └→ token_version 校验
  ↓
v2SignMiddleware (第3层: 签名)
  ├→ X-Session 有效性校验
  ├→ X-Sign 签名校验
  └→ Nonce 防重放
  ↓
...
| 操作 | 版本变化 | 效果 |
|---|---|---|
| 每次登录 | +1 | 新登录有效, 旧 token 仍可使用 |
| 修改密码 | +3 | **强制全设备下线** |

服务端校验: `token_version >= expected_version` 时放行。

### 50.3 V2RequireSignature 灰度

```
V2RequireSignature = false (当前灰度状态):
  ├→ 有签名 → 必须校验, 校验失败返回 401
  └→ 无签名 → 放行 (兼容旧客户端)
```
